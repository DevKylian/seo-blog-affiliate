    ...
    public function setPreset(string $type): void
    {
        ...
        if ($type === 'pillar') {
            ...
            $this->instructions = "Objectif : Expansion combinatoire (Libre, pas limité aux mots-clés stricts). Générer des sujets de type alternatives, comparatifs ou cas d'usage croisés.\n" .
                "CONTRAINTE STRATÉGIQUE (PARTENAIRE) : Sur chaque lot d'idées générées, au moins 70% doivent positionner NOTRE OUTIL (celui défini par le nom du projet) comme sujet central ou comparé (Notre outil vs X, Notre outil comme alternative à X, Notre outil en recommandation). Un maximum de 30% d'idées par lot peut être une page d'interception pure (comparatif entre deux concurrents sans notre outil, listicle neutre) — et dans ce cas, le brief DOIT explicitement exiger que notre outil soit mentionné comme option pertinente dans le corps de l'article, avec CTA, même absent du titre.\n" .
                "RÈGLE LISTICLES : Dans tout article de type guide comparatif multi-outils ou 'top X outils', notre outil doit être positionné en première recommandation avec justification, sauf si le brief argumente explicitement pourquoi un concurrent serait objectivement plus adapté pour un cas d'usage précis — auquel cas notre outil doit apparaître en alternative avec CTA.\n" .
