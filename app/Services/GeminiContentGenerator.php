...
class GeminiContentGenerator
{
    ...
    public function generate(SeoProject $project, string $type, ?Keyword $keyword, string $instructions = '', ?array $lockedBlueprint = null, ?string $lockedTitle = null, ?int $ignoreArticleId = null, ?string $model = null): Article
    {
        ...
        $preflight = $lockedBlueprint
            ? $this->duplicates->analyzeBlueprint($project, $lockedBlueprint, $ignoreArticleId, $type, $keyword)
    ...
    public function generatePartFromIdea(SeoProject $project, EditorialIdea $idea, string $instructions, int $step, int $attempt = 0, array $previousParts = []): string
    {
        ...
        if ($step === 0) {
            $preflight = $this->duplicates->analyzeBlueprint($project, $idea->blueprint(), null, $idea->content_type, $idea->keyword);
            if ($preflight['article'] && in_array($preflight['decision'], ['block', 'merge_or_reangle'], true)) {
                throw new DuplicateContentException($preflight['article'], $preflight['score'], $preflight['decision']);
            }
        }
...
- RÈGLE D'EXCLUSION MAJEURE (INDY ET ASSOCIATIONS) : Indy NE GÈRE PAS les associations loi 1901. Si tu dois écrire un article ciblant les associations, IL EST STRICTEMENT INTERDIT de recommander Indy ou de construire un guide d'utilisation Indy pour elles. Tu DOIS explicitement annoncer qu'Indy n'est pas adapté et tu DOIS recommander de vraies solutions alternatives (comme HelloAsso, AssoConnect, etc.) ou orienter le texte sur la comptabilité d'association en général SANS placer Indy comme solution.
- COMPATIBILITÉ ET STATUTS JURIDIQUES : Si l'outil n'est pas explicitement compatible avec une activité ou un régime fiscal dans les preuves, refuse toute promesse commerciale. Indique clairement son incompatibilité dès l'introduction et ne construis jamais un guide d'utilisation sur une prémisse fausse.
- INTERDICTION ABSOLUE DES CITATIONS ACADÉMIQUES : N'utilise STRICTEMENT JAMAIS de balises de type [S1], [1], (Source 1), ni de bibliographie ou section de sources. Toute affirmation factuelle doit rester vérifiable via une intégration invisible et naturelle dans le texte (ex: « selon les tarifs officiels »).
- Utilise exclusivement les preuves. N’invente ni prix, ni fonctionnalité, ni résultat observé.
- SIMULATION ET HYPOTHÈSES : Ne mélange jamais le délai d'attente avec le temps de travail effectif. Base-toi uniquement sur des heures de travail administratif. Si tu crées une simulation chiffrée (ex: 2 600 € de gain), tu dois OBLIGATOIREMENT utiliser les mots « estimation », « hypothèse » ou « à titre indicatif » à CHAQUE réutilisation de ce chiffre dans le texte. Ne le présente jamais comme un résultat garanti.
- UPSELL ET CTA : Les appels à l'action doivent être clairs et incitatifs, mais fondés UNIQUEMENT sur des avantages vérifiés dans les sources. Aucune affirmation de supériorité globale, de classement ("N°1") ou de superlatif non étayé n'est autorisée. Contextualise les fonctionnalités premium avec précision (ex: "Inclus dès le plan Plus à 9 €/mois").
...
2. INTERDICTION ABSOLUE DES CITATIONS ACADÉMIQUES : N'utilise STRICTEMENT JAMAIS de balises de type [S1], [1], (Source 1), ni de bibliographie ou section de sources. Chaque affirmation factuelle doit être vérifiable dans les preuves fournies, mais le texte final ne doit jamais afficher de marqueur de source direct. Intègre l'information naturellement (ex: "selon les chiffres officiels").
...
- IMAGES ET CRÉDITS : n'ajoute absolument JAMAIS de mentions de type "(Photo: Pexels)". Interdiction absolue d'utiliser le titre de la section comme attribut 'alt' de l'image ou comme balise <figcaption>. Le texte de la légende (figcaption) doit être original, informatif, ou la balise <figcaption> ne doit pas être générée.
