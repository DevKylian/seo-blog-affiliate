    ...
    <section class="automation-steps">
        <article class="panel automation-step {{ $workspaceReady ? 'done' : 'active' }}">
            ...
            <div class="automation-form">
                ...
                @if($mode === 'existing')
                    <div class="field full"><label>Projet existant</label><select wire:model="existingProjectId"><option value="">Sélectionner un projet</option>@foreach($projects as $existing)<option value="{{ $existing->id }}">{{ $existing->name }}</option>@endforeach</select></div>
                @else
                    <div class="field"><label>Nom de l’outil</label><input wire:model="name" placeholder="ex. Semrush"></div><div class="field"><label>Site officiel</label><input wire:model="websiteUrl" type="url" placeholder="https://..."></div>
                    <div class="field"><label>Page tarifs</label><input wire:model="pricingUrl" type="url" placeholder="https://.../pricing"></div><div class="field"><label>Lien affilié</label><input wire:model="affiliateUrl" type="url" placeholder="https://...?ref=..."></div>
                    <div class="field mini"><label>Pays</label><input wire:model="country" maxlength="2"></div><div class="field mini"><label>Devise</label><input wire:model="currency" maxlength="3"></div>
                @endif
                <div class="field full"><label>Autres pages officielles — une URL par ligne</label><textarea wire:model="extraSourceUrls" rows="3" placeholder="https://.../features&#10;https://.../integrations&#10;https://.../faq"></textarea><p class="field-help">Maximum 4 pages supplémentaires. Le site et la page tarifs sont inclus automatiquement.</p></div>
                <div class="field upload-field"><label>Export Semrush ou Google Keyword Planner</label><input wire:model="csv" type="file" accept=".csv,.tsv,.txt,text/csv,text/tab-separated-values"><p class="field-help"><a href="/examples/semrush-template.csv">Télécharger le modèle CSV</a></p></div>
                <div class="field"><label>Clé Gemini {{ $hasApiKey ? '(déjà configurée)' : '' }}</label><input wire:model="apiKey" type="password" autocomplete="new-password" placeholder="{{ $hasApiKey ? 'Laisser vide pour conserver la clé' : 'Clé API Gemini' }}"><p class="field-help">Chiffrée côté serveur dès l’enregistrement.</p></div>
                <div class="field full automation-keyword-paste"><label>Mots-clés Semrush à coller — facultatif en plus du fichier</label><textarea wire:model="pastedKeywords" rows="6" placeholder="Mot clé&#9;Intention&#9;Volume&#9;Tendance&#9;KD %&#9;CPC (EUR)&#10;logiciel devis facture&#9;I&#9;2400&#9;—&#9;54&#9;6,02"></textarea><p class="field-help">Copiez aussi la ligne d’en-tête Semrush. Si un fichier est ajouté, les deux sources sont fusionnées et les doublons mis à jour.</p></div>
                <button class="primary-button automation-submit" wire:click="prepare" type="button" @disabled($sourcesCollecting || ($run && in_array($run->status,['pending','processing'])))><span wire:loading.remove wire:target="prepare">{{ $sourcesCollecting ? 'Collecte en arrière-plan…' : 'Analyser et préparer le dossier →' }}</span><span wire:loading wire:target="prepare">Mise en file des sources…</span></button>
            </div>
        </article>
